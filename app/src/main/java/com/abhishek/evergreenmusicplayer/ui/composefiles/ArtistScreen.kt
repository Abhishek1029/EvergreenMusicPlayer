...
import com.abhishek.evergreenmusicplayer.ui.viewmodel.ArtistViewModel
...
fun RenderArtists(
    ...
    val artistState = artistViewModel.artistFlow.collectAsStateWithLifecycle()
    LazyVerticalGrid(
        columns = GridCells.Fixed(2),
        horizontalArrangement = Arrangement.SpaceEvenly,
        verticalArrangement = Arrangement.Top
    ) {
        items(artistState.value) { item ->
            Surface(
                Modifier
                    .padding(4.dp)
                    .clickable {
